...
void Shutdown()
{
    ...
    RenameThread("footycash-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: footycash.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: footycashd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizefootycashamount=<n> " + _("Keep N FootyCash anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. FootyCash is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. FootyCash is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("FootyCash version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "FootyCash server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of FootyCash") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart FootyCash to complete") << "\n";
    ...
    nAnonymizeFootyCashAmount = GetArg("-anonymizefootycashamount", 0);
    if(nAnonymizeFootyCashAmount > 999999) nAnonymizeFootyCashAmount = 999999;
    if(nAnonymizeFootyCashAmount < 2) nAnonymizeFootyCashAmount = 2;
    ...
    LogPrintf("Anonymize FootyCash Amount %d\n", nAnonymizeFootyCashAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1XFT+1000 == (.1XFT+100)*10
       10XFT+10000 == (1XFT+1000)*10
    */
