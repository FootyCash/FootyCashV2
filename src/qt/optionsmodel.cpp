...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeFootyCashAmount"))
        settings.setValue("nAnonymizeFootyCashAmount", 1000);
    nAnonymizeFootyCashAmount = settings.value("nAnonymizeFootyCashAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeFootyCashAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeFootyCashAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeFootyCashAmount:
            return QVariant(nAnonymizeFootyCashAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeFootyCashAmount:
            nAnonymizeFootyCashAmount = value.toInt();
            settings.setValue("nAnonymizeFootyCashAmount", nAnonymizeFootyCashAmount);
            emit AnonymizeFootyCashAmountChanged(nAnonymizeFootyCashAmount);
            break;
        default:
            break;
        }
