...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeFootyCashAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeFootyCashAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeFootyCashAmount = strAnonymizeFootyCashAmount.remove(strAnonymizeFootyCashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFootyCashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeFootyCashAmount*COIN) nMaxToAnonymize = nAnonymizeFootyCashAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeFootyCashAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeFootyCashAmount));
        strAnonymizeFootyCashAmount = strAnonymizeFootyCashAmount.remove(strAnonymizeFootyCashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFootyCashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeFootyCashAmount == 0){
