...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("footycash"))
        return false;
...
{
    // Convert footycash:// to footycash:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("footycash://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "footycash:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FootyCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for FootyCash.lnk
...
{
    return GetAutostartDir() / "footycash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a footycash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=FootyCash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the footycash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add footycash app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FootyCash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  footycash-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("FootyCash-Qt"));
