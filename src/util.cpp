...
int nAnonymizeFootyCashAmount = 1000;
...
#else
    const char* pszModule = "footycash";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FootyCash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FootyCash
    // Mac: ~/Library/Application Support/FootyCash
    // Unix: ~/.footycashv2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FootyCashV2";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "footycash.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "footycashd.pid"));
